...
import { TradeService } from '@/modules/swap/services/tradeService';
...
export function useSwapLogic(
  ...
  // Token States
  const [inputToken, setInputToken] = useState<BaseAsset | null>(null);
  const [outputToken, setOutputToken] = useState<BaseAsset | null>(null);
  ...
  const initializeTokens = useCallback(async () => {
    ...
    try {
      ...
      let initialInputToken: BaseAsset | null = null;
      let initialOutputToken: BaseAsset | null = null;
  ...
  // Fetch token balance
  const fetchBalance = useCallback(async (tokenToUse?: BaseAsset | null) => {
  ...
  // Fetch token price
  const getTokenPrice = useCallback(async (tokenToUse?: BaseAsset | null): Promise<number | null> => {
  ...
  const handleTokenSelected = useCallback(async (token: BaseAsset) => {
    if (!isMounted.current) return;
    ...
    try {
      ...
      let completeToken: BaseAsset | null = null;

      // Set up a timeout promise that will resolve after 8 seconds
      const timeoutPromise = new Promise<BaseAsset>(resolve => {
        setTimeout(() => {
