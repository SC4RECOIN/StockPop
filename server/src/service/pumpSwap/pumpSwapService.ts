...
export class PumpSwapClient {
  ...
  constructor() {
    ...
    console.log('[PumpSwapClient] Initializing with EXPO_PUBLIC_RPC_URL:', rpcUrl);
    ...
    try {
      ...
      try {
        ...
        if (quoteError.message && quoteError.message.includes('api key not found')) {
          console.error("[PumpSwapClient:buildSwapTx] API key error detected. Check your EXPO_PUBLIC_RPC_URL in environment variables.");
      ...
      if (error.message && error.message.includes('api key not found')) {
        ...
        console.error('[PumpSwapClient:buildSwapTx] Please add an API key to your EXPO_PUBLIC_RPC_URL in environment variables.');

        // Suggest fix in logs
        console.error('[PumpSwapClient:buildSwapTx] Example fix: Set EXPO_PUBLIC_RPC_URL=https://your-endpoint.example.com?api-key=YOUR_API_KEY in .env file');
      }
  ...
  async buildCreatePoolTx(params: CreatePoolParams): Promise<PumpSwapResponse<{ transaction: string }>> {
    try {
      ...
      const isMainnet = process.env.EXPO_PUBLIC_RPC_URL?.includes('mainnet') || false;
